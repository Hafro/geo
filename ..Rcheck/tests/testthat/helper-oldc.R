...
geo_oldc_compile <- function() {
  ...
  if(!file.exists(geo_c) || !file.exists(init_c)) return(NULL)
  cmd <- file.path(R.home("bin"), "R")
  args <- c("CMD", "SHLIB", geo_c, init_c, "-o", so)
  ...
  if(!file.exists(so)) return(NULL)
  so
}

geo_oldc_pointkriging <- function(lat, lon, z, xgr, vfit, maxnumber = 12,
                                  scale = "km", option = 1, maxdist = 0,
                                  rat = 3, nb = 8, set = 0, areas = 0,
                                  varcalc = FALSE, sill = 0, minnumber = 2,
                                  suboption = 1, outside = TRUE, degree = 0,
                                  lognormal = FALSE, zeroset = FALSE) {
  so <- geo_oldc_compile()
  if(is.null(so)) return(NULL)
  dyn.load(so)
  on.exit(dyn.unload(so), add = TRUE)

  if(is.null(vfit$rang1)) vfit$rang1 <- vfit$range
  ...
  if(length(areas) > 1) {
    ...
    for(i in seq_len(nareas)) {
      ...
      inn <- .C("marghc",
                as.double(lon), as.double(lat), as.integer(length(lat)),
                as.double(border$lon), as.double(border$lat),
                as.integer(length(border$lat)), as.integer(border$lxv),
                as.integer(length(border$lxv)), as.integer(inn),
                as.double(a), as.double(a1))
      isub <- inn[[9]] * i + isub
      inn1 <- .C("marghc",
                 as.double(lon1), as.double(lat1), as.integer(length(lat1)),
                 as.double(border$lon), as.double(border$lat),
                 as.integer(length(border$lat)), as.integer(border$lxv),
  ...
  if(option == 4) {
    if(maxdist == 0) maxdist <- vfit$rang1
  ...
  if(varcalc && sill == 0) sill <- vfit$sill
  xy <- 0

  out <- .C("c_pointkriging",
            as.double(lat), as.double(lon), as.double(z), as.integer(ndata),
            as.double(lat1), as.double(lon1), as.double(zgr),
            as.integer(length(lat1)), as.integer(reitur), as.integer(n),
            as.integer(m), as.integer(pts.in.reit), as.integer(npts.in.reit),
            as.integer(maxnumber), as.double(vgr), as.integer(stdcrt),
            as.integer(stdrrt), as.integer(dir), as.integer(i1),
            as.integer(length(i1)), as.integer(option), as.integer(inni),
            as.double(cov), as.double(rhgtside), as.double(x),
            as.integer(indrt), as.integer(jrt), as.integer(maxrt),
            as.integer(treitur), as.double(rat), as.double(maxdist),
            as.double(mz), as.integer(isub), as.integer(isub1),
            as.integer(subareas), as.double(variance), as.integer(varcalc),
            as.double(rhgtsbck), as.double(sill), as.integer(minnumber),
            as.integer(suboption), as.integer(xy), as.integer(d),
            as.double(lagrange), as.integer(zeroset))
