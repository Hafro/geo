...
#' @param cache Logical, if TRUE reuse clipped results for identical inputs.
...
geolines <-
  function(
    ...
    if (!outside) {
      ...
      if (cache) {
        ns <- asNamespace("geo")
        if (!exists(".geo_cache", envir = ns, inherits = FALSE)) {
          assign(".geo_cache", new.env(parent = emptyenv()), envir = ns)
        }
        cache_env <- get(".geo_cache", envir = ns)
        entry <- cache_env$geolines
        if (
          !is.null(entry) &&
            identical(entry$xx, xx) &&
            identical(entry$border, border)
        ) {
          xx <- entry$clipped
        } else {
          clipped <- findline(xx, border)
          cache_env$geolines <- list(
            xx = xx,
            border = border,
            clipped = clipped
          )
