...
geo_point_in_multipolygon <- function(x, y, border) {
  if (is.null(border$lxv)) {
    ...
    return(geo_point_in_polygon(x, y, poly$x, poly$y))
  }
  lxv <- border$lxv
  if (length(lxv) < 2) {
    poly <- geo_close_polygon(border$lon, border$lat)
  ...
  for (i in 1:(length(lxv) - 1)) {
    ...
    if (start <= end) {
      ...
      ring_inside <- geo_point_in_polygon(x, y, poly$x, poly$y)
...
geo_segment_intersections <- function(p1, p2, poly_x, poly_y) {
  ...
  x2 <- p2[1]
  y2 <- p2[2]
  res <- data.frame(t = numeric(0), x = numeric(0), y = numeric(0))
  for (i in 1:(n - 1)) {
    x3 <- poly_x[i]
    y3 <- poly_y[i]
    x4 <- poly_x[i + 1]
    ...
    if (t >= 0 && t <= 1 && s >= 0 && s <= 1) {
      xi <- x1 + t * (x2 - x1)
      yi <- y1 + t * (y2 - y1)
      res <- rbind(res, data.frame(t = t, x = xi, y = yi))
    }
  }
  res
}
