...
options(warn = 1)
base::assign(".ExTimings", "geo-Ex.timings", pos = 'CheckExEnv')
base::cat(
...
names(apply.shrink(
  depthloc$z,
  list(a = round(depthloc$lat), b = round(depthloc$lon)),
  mean
))
names(apply.shrink(
  depthloc$z,
  list(round(depthloc$lat), round(depthloc$lon)),
  mean,
  names = c("a", "b", "z")
))
...
geoplot(
  xlim = list(lat = c(64.85, 65.65), lon = c(-24.6, -21.7)),
  country = bisland,
  grid = FALSE
)
...
function(data) {
  ...
  i <- 1
  ind <- geoinside(tmpdata, reg = reg.lump[[i]], option = 0, robust = FALSE)
  if (length(ind) > 0) {
    tmpdata[ind, "area"] <- i
  }
  i <- 2
