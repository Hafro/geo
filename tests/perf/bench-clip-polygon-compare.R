...
clip_cpp <- getFromNamespace("geo_clip_polyline_to_polygon_cpp", "geo")

run_cpp <- function() {
  clip_cpp(line$x, line$y, border$x, border$y)
}

run_r <- function() {
  geo_clip_polyline_to_polygon_local(line$x, line$y, border$x, border$y)
}
...
n <- 25
elapsed_cpp <- numeric(n)
elapsed_r <- numeric(n)
for (i in 1:n) {
  elapsed_cpp[i] <- system.time(run_cpp())[["elapsed"]]
  elapsed_r[i] <- system.time(run_r())[["elapsed"]]
}

cat("CPP elapsed seconds (n=", n, ")\n", sep = "")
print(elapsed_cpp)
cat("CPP summary\n")
print(summary(elapsed_cpp))

cat("\nR elapsed seconds (n=", n, ")\n", sep = "")
print(elapsed_r)
cat("R summary\n")
print(summary(elapsed_r))
